...
function isInsideCircle(circle, point) {
  const pX = (point.x - circle.center.x) ** 2;
  const pY = (point.y - circle.center.y) ** 2;
  const d = Math.sqrt(pX + pY);
...
function getMatrixProduct(m1, m2) {
  const mOne = m1.length;
  const mTwo = m2.length;
  const columsTwo = m2[0].length;
  const result = [];
  for (let i = 0; i < mOne; i += 1) result[i] = [];
  for (let k = 0; k < columsTwo; k += 1) {
    for (let i = 0; i < mOne; i += 1) {
      let count = 0;
      for (let j = 0; j < mTwo; j += 1) count += m1[i][j] * m2[j][k];
      result[i][k] = count;
    }
  }
  return result;
